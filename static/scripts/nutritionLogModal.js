...
import { savedFoods } from "./globalNutritionData.js";
document.addEventListener('DOMContentLoaded', function () {
    ...
    const responseHTMLFormElement = document.getElementById('id_response_html');
    if (!responseHTMLFormElement)
        return;
    const responseHTMLFormData = responseHTMLFormElement.closest('p');
    if (!responseHTMLFormData)
        return;
    responseHTMLFormData.classList.add('hidden-food');
    const modal = document.getElementById('foodModal');
    if (!modal)
        return;
    const _savedFoods = savedFoods;
    if (!_savedFoods)
        return;
    // -------------------------------------------------------------------------
    // Error view
    ...
    function clearEditState() {
        ...
        if (caloriesInput)
            caloriesInput.value = '';
        const proteinInput = document.getElementById('customProtein');
        if (proteinInput)
            proteinInput.value = '';
    ...
    openButton.addEventListener('click', () => {
        ...
        if (myFoodTabBtn)
            myFoodTabBtn.classList.add('active');
        const myFoodPanel = document.getElementById('tab-myfood');
        if (myFoodPanel)
            myFoodPanel.style.display = 'block';
    ...
    function openEditModal(item) {
        ...
        const proteinInput = document.getElementById('customProtein');
        if (proteinInput)
            proteinInput.value = item.dataset.protein ?? '';
        const fatInput = document.getElementById('customFat');
        if (fatInput)
            fatInput.value = item.dataset.fat ?? '';
        const carbsInput = document.getElementById('customCarbs');
        if (carbsInput)
            carbsInput.value = item.dataset.carbs ?? '';
        const addBtn = document.getElementById('addCustomFood');
        if (addBtn)
    ...
    function appendFoodItem(food) {
        ...
        if (nameElement)
            nameElement.addEventListener('click', () => openEditModal(item));
        if (macrosElement)
            macrosElement.addEventListener('click', () => openEditModal(item));
    ...
    window.updateFoodHTML = function () {
        const responseInput = document.getElementById('id_response_html');
        const foodList = document.getElementById('foodList');
    ...
    window.updateTotals = function () {
        ...
        if (caloriesInput)
            caloriesInput.value = totals.calories.toFixed(1);
        if (proteinInput)
        ...
        if (totalProteinPct)
            totalProteinPct.textContent = '(' + protein_pct.toFixed(1) + '%)';
        if (totalFatPct)
            totalFatPct.textContent = '(' + fat_pct.toFixed(1) + '%)';
        if (totalCarbsPct)
            totalCarbsPct.textContent = '(' + carb_pct.toFixed(1) + '%)';
    };
    // -------------------------------------------------------------------------
    ...
    if (savedHTML.trim()) {
        const foodList = document.getElementById('foodList');
        if (foodList)
            foodList.innerHTML = savedHTML;
        document.querySelectorAll('.food-list-item').forEach(item => {
            const removeBtn = item.querySelector('.food-list-remove');
            if (removeBtn) {
    ...
    const myFoodSearch = document.getElementById('myFoodSearch');
    if (myFoodSearch) {
    ...
    if (addCustomFoodBtn) {
        addCustomFoodBtn.addEventListener('click', () => {
            ...
            const fatInput = document.getElementById('customFat');
            const carbsInput = document.getElementById('customCarbs');
            if (!nameInput || !caloriesInput || !proteinInput || !fatInput || !carbsInput)
                return;
            ...
            if (!name) {
                showModalError('Please enter a food name.');
                return;
            }
            const numericFields = [
